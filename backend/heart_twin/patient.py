...
class PatientProfile:
    ...
    def ventricular_volumes(self) -> VentricularVolumes:
        """Resolve a volume pair using entered LVEF as the baseline target."""
        if self.edv_ml is not None and self.esv_ml is not None:
            esv = self.edv_ml * (1.0 - self.lvef_percent / 100.0)
            return VentricularVolumes(
                self.edv_ml,
                esv,
                "reported",
                "EDV entered by the clinician; ESV derived from the entered LVEF.",
            )
