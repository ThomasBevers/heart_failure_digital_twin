...
def _calibrate_emax(patient: PatientProfile, volumes: VentricularVolumes) -> float:
    ...
    target_ef = volumes.ejection_fraction_percent
    low, high = 0.10, 5.00
    neutral = ScenarioControls()
    for _ in range(16):
        candidate = (low + high) / 2.0
        parameters = _make_parameters(patient, volumes, emax=candidate, controls=neutral)
        predicted_ef = _run(patient, volumes, parameters, neutral).summary.ejection_fraction_percent
